<x-dynamic-component :component="$getFieldWrapperView()" :field="$field" class="relative z-50">

    @php
        $textareaID = 'tiny-editor-' . str_replace(['.', '#', '$'], '', $getId());
    @endphp

    <div x-data="{ state: $wire.{{ $applyStateBindingModifiers("\$entangle('{$getStatePath()}')") }}, initialized: false }" x-load-css="[@js(\Filament\Support\Facades\FilamentAsset::getStyleHref('tiny-css', package: 'amidesfahani/filament-tinyeditor'))]" x-load-js="[@js(\Filament\Support\Facades\FilamentAsset::getScriptSrc($getLanguageId(), package: 'amidesfahani/filament-tinyeditor'))]" x-init="(() => {
        $nextTick(() => {
            ...
            tinymce.init({
                ...
                menu: {
                    file: { title: 'File', items: 'restoredraft | preview | print | deleteallconversations' },
                    edit: { title: 'Edit', items: 'undo redo | cut copy paste pastetext | selectall | searchreplace' },
                    view: { title: 'View', items: 'code | visualaid visualchars visualblocks | spellchecker | preview fullscreen | showcomments' },
                    insert: { title: 'Insert', items: 'image link media addcomment pageembed codesample inserttable | charmap hr | pagebreak nonbreaking anchor tableofcontents | insertdatetime' },
                    ...
                    editor.ui.registry.addButton('customInsertButton', {
                        ...
                        onAction: function(_) {
                            fetch('/get-document')
                                ...
                                .catch(error => console.error('Error fetching template file:', error));

                            // Stop the event propagation to prevent it from being triggered twice
                            return false;
                        }
                    });
        ...
        @else
            <div x-html="state" style="z-index: 1050;"
